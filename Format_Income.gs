 ...
 */
function получитьДатуФорматирование(значение) {
  ...
  if (typeof значение === 'string') {
    ...
    if (части.length === 3) {
      var день = parseInt(части[0], 10);
      var месяц = parseInt(части[1], 10) - 1; // месяцы в JS начинаются с 0
      var год = parseInt(части[2], 10);
      var дата = new Date(год, месяц, день);
...
function получитьНомерМесяца(дата) {
  ...
  if (!(дата instanceof Date)) {
    Logger.log('Дата не является объектом Date: ' + typeof дата);
    return null;
  }
  
  if (isNaN(дата.getTime())) {
    Logger.log('Невалидная дата: ' + дата);
    return null;
  }
  
  var месяц = дата.getMonth(); // 0 = январь, 11 = декабрь
  return месяц;
}
...
function Format_Income() {
  try {
    ...
    try {
      ...
      spreadsheet.toast(error.message, 'Ошибка', 5);
      return;
    }
    ...
    if (последняяСтрока < КОНФИГ_ФОРМАТИРОВАНИЯ.ПЕРВАЯ_СТРОКА_ДАННЫХ) {
      ...
      spreadsheet.toast('На листе "' + КОНФИГ_ФОРМАТИРОВАНИЯ.ЛИСТ_ДАННЫХ + '" нет данных для форматирования', 'Информация', 5);
      return;
    }
    ...
    // Очищаем форматирование начиная со второй строки
    if (последняяСтрока >= КОНФИГ_ФОРМАТИРОВАНИЯ.ПЕРВАЯ_СТРОКА_ДАННЫХ) {
      var количествоСтрок = последняяСтрока - КОНФИГ_ФОРМАТИРОВАНИЯ.ПЕРВАЯ_СТРОКА_ДАННЫХ + 1;
      var диапазонОчистки = лист.getRange(
        КОНФИГ_ФОРМАТИРОВАНИЯ.ПЕРВАЯ_СТРОКА_ДАННЫХ, 
        1, 
        количествоСтрок, 
        последняяКолонка
      );
      диапазонОчистки.clearFormat();
      Logger.log('Очищено форматирование для ' + количествоСтрок + ' строк (строки ' + КОНФИГ_ФОРМАТИРОВАНИЯ.ПЕРВАЯ_СТРОКА_ДАННЫХ + '-' + последняяСтрока + ')');
    }
    ...
    for (var i = 0; i < значенияДат.length; i++) {
      ...
      if (!датаЗначение || датаЗначение === '') {
        continue;
      }
      
      var дата = получитьДатуФорматирование(датаЗначение);
    ...
    // Логируем статистику по месяцам
    for (var месяц in строкиПоМесяцам) {
      Logger.log('Месяц ' + названияМесяцев[parseInt(месяц)] + ': ' + строкиПоМесяцам[месяц].length + ' строк');
    }
    ...
    for (var номерМесяца in строкиПоМесяцам) {
      ...
      Logger.log('Форматирование ' + строкиМесяца.length + ' строк для ' + названияМесяцев[parseInt(номерМесяца)] + ' (цвет: ' + цвет + ')');
      
      // Сортируем строки по возрастанию для правильной группировки
      строкиМесяца.sort(function(a, b) { return a - b; });
      ...
      for (var j = 1; j < строкиМесяца.length; j++) {
        ...
        if (текущаяСтрока === предыдущаяСтрока + 1) {
          ...
          if (текущаяГруппа.length > 0) {
            ...
            Logger.log('Применен цвет ' + цвет + ' к строкам ' + началоГруппы + '-' + (началоГруппы + количествоСтрок - 1));
    ...
    var сообщение = 'Форматирование завершено! Обработано строк: ' + обработаноСтрок + ', месяцев: ' + Object.keys(строкиПоМесяцам).length;
    spreadsheet.toast(сообщение, 'Готово', 5);
    ...
    try {
      SpreadsheetApp.getActiveSpreadsheet().toast(сообщение, 'Ошибка', 10);
    } catch (e) {
      // Если toast тоже не работает, просто логируем
      Logger.log('Не удалось показать сообщение пользователю: ' + e.toString());
    }
    throw error;
  }
}
...
function onOpen() {
  ...
  ui.createMenu('📊 Финансы')
    ...
    .addSeparator()
    .addItem('Формат ВСЕ', 'Format_Income')
    .addToUi();
}
